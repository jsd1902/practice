 ...
 * 一条员工信息，格式如:1,张三,25,男,5000,2006-12-12
...
public class Test02 {
	public static void main(String[] args) {
		...
		while(true){
			...
			if(num<5){
				System.out.println("人数至少为5，请重新输入！");
				continue;
			}
			break;
		}
		try {
			Collection<Emp> emps = getEmp(num);
	...
	public static void writeXml(Collection<Emp> emps,String fileName){
		...
		try {
			...
			for(Emp emp : emps){
				...
				empEle.addElement("hiredate").addText(emp.getHiredate());
			}
